...
import project.seatsence.src.reservation.service.TempReservationService;
...
public class AdminReservationApi {
    ...
    @Operation(summary = "admin 예약 상태 변경하기",
            description = "예약 상태는 '승인', '거절' 중 하나로 선택")
    @PostMapping("/{store-id}/")
    public void setReservationStatus(@PathVariable("store-id") Long storeId,
                                     @RequestParam("reservation-id") Long reservationId,
                                     @RequestBody ReservationStatus reservationStatus){
        tempReservationService.setReservationStatus(reservationId, reservationStatus);
    }
}